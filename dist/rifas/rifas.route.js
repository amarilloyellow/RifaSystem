// src/rifas/rifas.route.ts
//importaciones
import { Router } from "express";
import { createRifa, createTicketsForRifa, getAllRifas, getRifaById, getTicketsByRifaId, markTicketAsSold, updateRifa } from "./rifas.service.js";
// instancia del router
const rifasRouter = Router();
// Ruta para obtener todas las rifas
rifasRouter.get('/', async (req, res) => {
    const rifas = await getAllRifas();
    res.json(rifas);
});
// Crear una nueva rifa
rifasRouter.post('/', async (req, res) => {
    const rifaData = req.body;
    try {
        const nuevaRifa = await createRifa(rifaData);
        res.status(201).json(nuevaRifa);
    }
    catch (error) {
        res.status(400).json({ error: 'Error al crear la rifa' });
    }
});
// Actualizar una rifa
rifasRouter.patch('/:id', async (req, res) => {
    const rifaId = req.params.id;
    const rifaData = req.body;
    try {
        const updatedRifa = await updateRifa(rifaId, rifaData);
        res.json(updatedRifa);
    }
    catch (error) {
        res.status(400).json({ error: 'Error al actualizar la rifa' });
    }
});
// Obtener una rifa por ID 
rifasRouter.get('/:id', async (req, res) => {
    const rifaId = req.params.id;
    try {
        const rifa = await getRifaById(rifaId);
        if (rifa) {
            res.json(rifa);
        }
        else {
            res.status(404).json({ error: 'Rifa no encontrada' });
        }
    }
    catch (error) {
        res.status(400).json({ error: 'Error al obtener la rifa' });
    }
});
// Crear tickets para una rifa
rifasRouter.post('/:id/tickets', async (req, res) => {
    const rifaId = req.params.id;
    const ticketData = req.body; // { number: number, ticketHolderID: string, ticketHolderEmail: string, ticketHolderName: string, ticketHolderPhone: string }
    try {
        const newTicket = await createTicketsForRifa(rifaId, ticketData);
        res.status(201).json(newTicket);
    }
    catch (error) {
        res.status(400).json({ error });
    }
});
// Obtener todos los tickets de una rifa
rifasRouter.get('/:id/tickets', async (req, res) => {
    const rifaId = req.params.id;
    try {
        const tickets = await getTicketsByRifaId(rifaId);
        res.json(tickets);
    }
    catch (error) {
        res.status(400).json({ error: 'Error al obtener los tickets de la rifa' });
    }
});
// definir tickets como vendidos
rifasRouter.patch('/:ticketId/tickets/sell', async (req, res) => {
    const { ticketId } = req.params;
    try {
        const soldTicket = await markTicketAsSold(ticketId);
        res.json(soldTicket);
    }
    catch (error) {
        res.status(400).json({ error });
    }
});
// exportar el router
export default rifasRouter;
//# sourceMappingURL=rifas.route.js.map